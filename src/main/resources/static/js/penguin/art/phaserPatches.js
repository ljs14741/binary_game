/* Phaser 런타임 패치: 고해상도 텍스처의 논리 크기 보정, 텍스트 기본 해상도 */
import Phaser from 'phaser';
import { DPR } from './dpr.js';

for (const Cls of [Phaser.GameObjects.Image, Phaser.GameObjects.Sprite]) {
...
// WebGL 스프라이트 배칭은 텍스처 해상도를 무시하고 실제 픽셀 크기로 사각형을 그림 (기준점은 위 패치로 논리 크기).
// 그대로 두면 DPR 2 기기에서 그림이 2배로 커지고 반 칸 밀림 → 그리는 순간에만 크기·기준점을 해상도만큼 맞춰줌
{
